...
fn set_content_type() {
    ...
    let request = test_custom_headers(set);
    let content_type = request.headers().get::<ContentType>().expect("No ContentType");
...
fn set_content_length() {
    ...
    let request = test_custom_headers(set);
    let content_length = request.headers().get::<ContentLength>().expect("No ContentLength");
    ...
    S: FnOnce(&mut HttpHandle)
{
    ...
    server.requests.recv_timeout(Duration::from_secs(1)).unwrap()
}
