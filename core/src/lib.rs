...
pub trait Transport: Sized {
    ...
    /// A stream of strings which represent a single JSON value that is either an array or an
    /// object used to receive messages from a JSON-RPC server.
    type Stream: Stream<Item = String, Error = Self::Error>;
    /// A sink of strings which represent a single JSON value that is either an array or an
