...
error_chain! {
    errors {
        /// Time out error while performing an RPC request.
        RequestTimeout {
            description("RPC request has timed out")
        }
...
/// Represents an error that can be returned by a `Transport` for an RPC request.
///
/// In geral this is either a transport-specific error, or a request timeout.
pub trait RpcRequestError: ::std::error::Error + Send {
    /// Returns if this error was caused by a request timeout.
    fn is_timeout(&self) -> bool;
}

impl RpcRequestError for ::std::io::Error {
    fn is_timeout(&self) -> bool {
        self.kind() == ::std::io::ErrorKind::TimedOut
    }
}

impl<E: RpcRequestError + 'static> From<E> for Error {
    ...
    F: Future<Item = Vec<u8>, Error = E> + Send + 'static,
{
    ...
    fn poll(&mut self) -> futures::Poll<Self::Item, Self::Error> {
        let response_raw = try_ready!(self.transport_future.poll().map_err(Error::from));
        trace!(
...
pub trait Transport {
    ...
    type Error: RpcRequestError + 'static;
